...
import {
  SYMBOLS, PAYTABLE, SCATTER_PAY, FREE_SPINS, COLLECT, MONEY_VALUES,
  JACKPOTS, BET_LEVELS, DEFAULT_BET, MAX_WIN,
  REELS, ROWS, LINES, WILD, SCATTER, MONEY,
  WILD_REELS_BASE, WILD_REELS_FREE, jackpotAmount
} from '../games/mavimera/config.js';
import { PAYLINES } from '../games/mavimera/paylines.js';
import { playRound, ensureState, round2 } from '../games/mavimera/session.js';
import { ProvablyFairRng } from '../game/rng.js';
import {
  getMaviMeraPools, publicAccount, save, getSettings, addXp, recordHistory
...
const GAME_NAME = 'MAVİ MERA';

/** Bahis seviyesine göre jackpot merdiveni (istemci şeridi bunu gösterir). */
export function mavimeraJackpotView(totalBet = DEFAULT_BET) {
  const pools = getMaviMeraPools();
...
router.get('/mavimera/config', (req, res) => {
  ...
  res.json({
    ...
    symbols: SYMBOLS,
    wild: WILD,
    wildReelsBase: WILD_REELS_BASE,
    wildReelsFree: WILD_REELS_FREE,
    scatter: SCATTER,
    money: MONEY,
    paytable: PAYTABLE,
    ...
    collect: COLLECT,
    moneyValues: MONEY_VALUES.map(({ id, value, jackpot }) => ({ id, value, jackpot })),
    paylines: PAYLINES,
...
router.get('/mavimera/jackpots', (req, res) => {
  const bet = Number(req.query.bet) || DEFAULT_BET;
  res.json({ jackpots: mavimeraJackpotView(BET_LEVELS.includes(bet) ? bet : DEFAULT_BET) });
});

/** Oyuncunun bu oyuna ait durumu (bahis, kalan bedava dönüş, seviye). */
router.get('/mavimera/state', requireAccount, (req, res) => {
  const st = ensureState(req.account);
  res.json({
    state: { bet: st.bet, free: { ...st.free, win: round2(st.free.win) } },
    player: publicAccount(req.account),
    jackpots: mavimeraJackpotView(st.bet)
  });
});

router.post('/mavimera/bet', requireAccount, (req, res) => {
  const st = ensureState(req.account);
  ...
  st.bet = bet;
  save();
  res.json({ state: { bet: st.bet, free: st.free }, jackpots: mavimeraJackpotView(bet) });
});
...
/** Bir tur: çevirme + hat/dümen değerlendirmesi + balıkçı toplaması. */
router.post('/mavimera/spin', requireAccount, (req, res) => {
  ...
  const jackpotWin = round.collect?.jackpotWins?.[0] || null;
  ...
  res.json({
    ...
    jackpots: mavimeraJackpotView(round.bet)
  });
});
